...
async function handleMessages(sock, messageUpdate) {
  try {
    ...
    const isBotMessage = m.key?.id?.startsWith("BAE5") || m.key?.id?.startsWith("3EB0") || m.quoted?.key?.fromMe || ctx?.isForwarded && ctx?.forwardingScore > 0;
    ...
    if (m.isGroup) {
      ...
      if (group.banchat) {
        ...
        const cmd = isCommand ? text.slice(prefix.length).split(/\s+/)[0] : "";
        if (!isOwner) {
    ...
    if (!isCommand && global.noPrefix) {
      ...
      const exists = plugins.find(p => Array.isArray(p.command) ? p.command.includes(firstWord) : p.command === firstWord);
      if (exists) {
        isCommand = true;
        usedPrefix = "";
      }
    }
    const command = isCommand ? body.replace(usedPrefix, "").trim().split(/\s+/)[0]?.toLowerCase() : null;
...
async function handleMessagesExecutor(sock, m, message, plugins) {
  ...
  if (!user.registered && !isOwner) {
    const allow = ["daftar", "register", "sc", "script", "menu", "help", "unbanchat"];
  ...
  if (isGroup && (plugin.admin || plugin.botAdmin || plugin.ownerAdmin)) {
  ...
  if (plugin.ownerAdmin && !(isOwner || isSenderAdmin)) return m.reply("❌ Hanya owner atau admin grup")
  ...
  try {
    ...
    await m.reply("❌ Terjadi error saat menjalankan perintah" + e);
  }
  if (!isOwner) {
