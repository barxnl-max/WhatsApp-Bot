...
module.exports = {
  ...
  async handler({ m, sock }) {
    ...
    if (!["imageMessage", "videoMessage", "stickerMessage"].includes(mtype)) {
      return m.reply(
        "❌ Kirim / reply gambar, video, gif, atau sticker\n\n" +
          "Contoh:\n" +
          "• .s",
      );
    }
    ...
    try {
      ...
      if (isVideo && duration > 9) {
        fs.unlinkSync(input);
        return m.reply("❌ Video maksimal 9 detik");
      }

      const vf = "crop=min(iw\\,ih):min(iw\\,ih),scale=512:512";

      const cmd = isVideo
        ? `ffmpeg -y -i "${input}" -t 9 -vf "fps=10,${vf}" -c:v libwebp -loop 0 -pix_fmt yuva420p -quality 35 -compression_level 6 -an "${output}"`
        : `ffmpeg -y -i "${input}" -vf "${vf}" -c:v libwebp -loop 0 -pix_fmt yuva420p -quality 60 -compression_level 6 "${output}"`;
      ...
      if (webpBuf.length > 900 * 1024) {
        ...
        const smallCmd = isVideo
          ? `ffmpeg -y -i "${input}" -t 2 -vf "fps=8,${vf}" -c:v libwebp -loop 0 -pix_fmt yuva420p -quality 30 -compression_level 6 -b:v 80k "${small}"`
          : `ffmpeg -y -i "${input}" -vf "${vf}" -c:v libwebp -loop 0 -pix_fmt yuva420p -quality 45 -compression_level 6 "${small}"`;
      ...
      const json = {
        ...
        "sticker-pack-name": global.packname || "CATA BOT",
